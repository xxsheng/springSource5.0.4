...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	* */
	...
	/*
	* 	autowire-candidate属性设置为false，这样容器在查找自动装配对象时，将不考虑该bean，即它不会被考虑作为其他bean自动
	* 装配的候选者，但是该bean本身还是可以使用自动装配来注入其他bean的。
	* 对应的属性 autowire-candidate
	* */
	private boolean autowireCandidate = true;

	/*
	* 	自动装配时当出现多个bean候选者时，将作为首选者，对应bean属性 primary
	* */
	...
	* 如果为false，则存在如下情况
	* interface ITest{}
	* class ITestImpl implemants ITest {}
	* class Main {
	* 	Main(ITest I){}
	* 	Main(ITestImpl I){}
	* }
	* 程序抛出异常，因为spring无法确定定位哪个构造函数
	* 程序设置
	*
	* */
