 ...
 */
// 综合FactoryBeanRegistrySupport 和 ConfigurableBeanFactory的功能
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		* 为什么首先会使用这段代码呢，
		* 因为在创建单列bean的时候会存在依赖注入的情况，而在创建的时候为了避免循环依赖，
		* spring创建beand的原则是不等bean创建完成就会将创建bean的ObjectFactory提早曝光
		...
		* */
		...
		if (sharedInstance != null && args == null) {
			...
			// 返回对应的实例，有时候存在诸如FactoryBean的情况并不是直接返回实例，而是返回指定方法返回的实例
			...
			/*
			* 只有在单例的情况下才会尝试解决循环依赖，原型模式下，如果存在A中有B的属性，B中有A的属性，那么当依赖注入的时候
			* 就会产生当A还未创建完的时候因为对于B的创建而再次返回创建A，造成循环依赖，也就是下面的情况
			* */
		...
		// 如果指定的需求类型不为空需要进行类型转换，否则直接强制转换---需要检查需要的类型是否符合bean的实际类型
	...
	* */
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		...
		// requiresDestrution 寻找destory-method或者PreDestroy
