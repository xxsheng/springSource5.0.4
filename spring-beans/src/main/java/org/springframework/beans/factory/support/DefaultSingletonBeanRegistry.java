...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	// 也是保存beanName和创建bean实例之间的关系，与singletonObjects的不同之处在于，当一个单例bean被放到这里之后，那么
	// 当bean还在创建过程中，就可以通过getBean方法获取到了，其目的是用来检测循环引用。
	...
	* */
			...
			synchronized (this.singletonObjects) {
				...
				if (singletonObject == null && allowEarlyReference) {
					...
					if (singletonFactory != null) {
						...
						// 记录在缓存中，earlySingletonObjects和singletonFactoies互斥
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		// 全局变量需要同步
		synchronized (this.singletonObjects) {
			// 首先检查对应的bean是否已经加载过，因为singleton模式其实就是复制已经创建的bean，所以这一步是必须的
			Object singletonObject = this.singletonObjects.get(beanName);
			// 单例池中如果没有则进行实例化
			if (singletonObject == null) {
				...
				// 重要代码 --- 记录bean正在创建
				// 记录加载状态，也就是通过this.singletonsCurrentlyInCreation.add(beanName)将正在要创建的bean记录在缓存中
				// 这样便可以对循环依赖进行检测
