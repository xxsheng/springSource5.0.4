...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsBeforeInitialization(Object existingBean, String beanName)
		...
		for (BeanPostProcessor beanProcessor : getBeanPostProcessors()) {
			// org.springframework.context.annotation.CommonAnnotationBeanPostProcessor调用初始化方法
	...
	public Object applyBeanPostProcessorsAfterInitialization(Object existingBean, String beanName)
		...
		Object result = existingBean;
		for (BeanPostProcessor beanProcessor : getBeanPostProcessors()) {
			// 调用bean的后置处理器，将bean放到各个位置中（？）,或者初始化（代理）
...
//			3、如果既不存在工厂方法也不存在带有参数的构造函数，则使用默认的构造函数进行bean的实例化。
		...
		try {
			...
			// 调用初始化方法，比如init-method。springbean生命周期回调函数 (此处可能有aop处理)(override-method处理)
		...
		if (earlySingletonExposure) {
			...
			// ****** earlySingletonReference 只有在检测到有循环依赖的情况下才不会为null
			if (earlySingletonReference != null) {
				// 如果exposedObject没有在初始化方法中被改变，也就是没有被增强
				if (exposedObject == bean) {
					// 在循环依赖的情况下，走到这一步的话，earlySingletonReference大概率为aop对象
					exposedObject = earlySingletonReference;
				}
				else if (!this.allowRawInjectionDespiteWrapping && hasDependentBean(beanName)) {
					// 当前bean并非最终bean，常见异步注解bean中的循环依赖
					...
					// 因为bean创建后其所依赖的bean一定是已经创建的，actualDependentBeans不为空则表示当前bean创建后其所依赖的bean
					// 却没有没全部创建完，也就是说存在循环依赖
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				// 推断对象类型，解决resolvedTargetType类型的填充
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// class如果有自定义的构造方法，则需要根据参数解析构造函数
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (mbd.getResolvedAutowireMode() == RootBeanDefinition.AUTOWIRE_BY_NAME ||
			...
			// 根据类型自动注入
			// 寻找bw中需要依赖注入的属性，然后遍历这些属性并寻找类型匹配的bean，其中最复杂的就是寻找类型匹配的bean，同时，spring中提供了对集合类型注入的支持
			// 如使用注解的方式 @Autowired private List<Impl>; spring会把所有与Impl类型匹配的找出来并注入到impls属性中，正是由于这一因素，所以在autowiredBType
			// 函数中，新建了局部遍历autowiredBeanNames，用于储存所有依赖的bean，如果只是对非集合类型的属性注入来说，此属性并无用处
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					...
					// 解析指定beanName的属性所匹配的值，并把解析到的属性名称存储在autowiredBeanNames中，当属性中有多个封装bean时如：
					// private List<Impl>将会找到所有Impl类型的bean并将其注入
	...
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			// ApplicationContextAware,ResourceLoaderAware,ServletContextAware等等
			invokeAwareMethods(beanName, bean);
		}

		Object wrappedBean = bean;
		if (mbd == null || !mbd.isSynthetic()) {
			// 对初始化方法进行处理init-method，PostConstruct
			wrappedBean = applyBeanPostProcessorsBeforeInitialization(wrappedBean, beanName);
		}

		try {
			// 处理 afterPropertiesSet方法和自定义初始化方法
