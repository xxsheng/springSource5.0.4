...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 准备刷新上下文环境
			// 初始化前的准备工作，例如对系统属性或者环境变量进行准备或验证
			// 在某种情况下项目的使用需要读取某些系统变量，而这个变量的设置很可能会影响着系统的正确性，那么
			// ClasspathXmlApplicationContext为我们提供的这个准备函数就显得非常必要，它可以在spring启动的时候提前对必需的变量
			// 进行存在性验证
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 处理beanDefinition（xml处理）
			// 不同的实现类处理不同的beanDefinition 例如 xml形式，注解形式

			// ClassPathXmlApplicationContext包含着BeanFactory所提供的一切特征，那么在这一步骤中将会复用BeanFactory中的配置文件
			// 读取解析及其他功能，在一步之后ClassPathXmlApplicationContext已经包含了BeanFactory所提供的功能，也就是可以进行Bean的提取
			// 等基础操作了
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			// 对beanFactory进行各种功能填充
			// @Qualifier 与 @Autowired	在这一步添加的支持
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// 子类覆盖方法对beanFactory做额外的处理
				...
				// 完成刷新过程，通知生命处理器lifecycleProcessor刷新过程，同时发出ContextRefreshEvent通知别人
				finishRefresh();
			}
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 刷新容器，对容器做一些初始化前的工作，判断容器是否存在，销毁等操作
		refreshBeanFactory();
		// 返回上一步创建的容器
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置beanFactory的表达式语言来处理，spring3增加了表达式语言的支持，
		// 默认使用#{bean.xxx}的形式来调用相关属性值
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		// 为beanFactory增加了一个默认的propertyEditor，这个主要是对bean属性d等设置管理的一个工具
	...
	* */
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		if (!beanFactory.hasEmbeddedValueResolver()) {
			beanFactory.addEmbeddedValueResolver(strVal -> AbstractApplicationContext.this.getEnvironment().resolvePlaceholders(strVal));
		}
	...
	protected void finishRefresh() {
		...
		// 当applicationContext启动或停止时，他会通过lifecycleProcessor来与所有声明的bean的周期状态更新
		...
		// 初始化注册事件
		// 当完成applicationContext初始化的时候，要通过spring中的事件发布机制发出ContextRefreshEvent事件，已
		// 确保对应的监听器可以做进一步的处理
