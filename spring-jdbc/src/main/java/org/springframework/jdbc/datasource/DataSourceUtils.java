...
public abstract class DataSourceUtils {
	...
	public static Connection getConnection(DataSource dataSource) throws CannotGetJdbcConnectionException {
		try {
			/*
			* 在数据库连接方面，Spring主要考虑的是关于事务方面的处理，基于事务处理的特殊性，Spring需要保证线程中的数据库操作都是使用
			* 同一个事务连接
			* */
	 ...
	 */
	/*
	* 数据库的连接释放并不是直接调用了Connection的API的close方法，考虑到存在事务的情况，如果当前线程存在事务，那么说明在当前线程中
	* 存在共用数据库连接，这种情况下直接使用ConnectionHolder的released方法进行连接数减1，而不是真正的释放连接。
	*
	* */
	...
	public static void doReleaseConnection(@Nullable Connection con, @Nullable DataSource dataSource) throws SQLException {
		...
		if (dataSource != null) {
			// 当前线程存在事务的情况下说明共用数据库连接直接使用ConnectionHolder中的released方法进行减1而不是真正的释放连接
			ConnectionHolder conHolder = (ConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);
